...
def initialize_user_vector(user: dict, category_to_index: Dict[str, int]) -> List[float]:
    ...
    # Handle both list format and object format with 'categories' key
...
def generate_learning_charts(user_trajectories: Dict[str, List[List[float]]], output_dir: Path) -> List[str]:
    """Generate learning trajectory charts for each user.
    ...
    for user_name, history in user_trajectories.items():
        if len(history) < 2:
            continue

        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8))
        fig.suptitle(f"Learning Trajectory: {user_name}", fontsize=14, fontweight='bold')

        steps = list(range(len(history)))

        # Plot 1: Category vector evolution over time
        for cat_idx, category in enumerate(CATEGORIES):
            values = [vec[cat_idx] for vec in history]
            ax1.plot(steps, values, label=category, color=colors[cat_idx % len(colors)], linewidth=2)

        ax1.set_xlabel("Engagement Step")
        ax1.set_ylabel("Interest Score")
        ax1.set_title("Category Interest Evolution")
        ax1.legend(loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=8)
        ax1.grid(True, alpha=0.3)
        ax1.set_ylim(0, 1.2)

        # Plot 2: Heatmap of final vs initial vector
        initial_vec = history[0]
        final_vec = history[-1]
        delta = [f - i for f, i in zip(final_vec, initial_vec)]

        x = range(len(CATEGORIES))
        bar_colors = ['#2ecc71' if d > 0 else '#e74c3c' for d in delta]
        ax2.bar(x, delta, color=bar_colors, edgecolor='black', linewidth=0.5)
        ax2.set_xticks(x)
        ax2.set_xticklabels([c.split(' & ')[0][:8] for c in CATEGORIES], rotation=45, ha='right', fontsize=8)
        ax2.set_xlabel("Category")
        ax2.set_ylabel("Change (Final - Initial)")
        ax2.set_title(f"Net Interest Change After {len(history)-1} Engagements")
        ax2.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
        ax2.grid(True, alpha=0.3, axis='y')

        plt.tight_layout()

        chart_filename = f"learning_trajectory_{user_name.lower()}.png"
...
def summarize_metrics() -> Tuple[str, Dict[str, List[List[float]]]]:
    """Generate metrics report and return user trajectories for charting."""
    ...
    ndcgs = []
    weighted_rates = []
    user_trajectories: Dict[str, List[List[float]]] = {}
    ...
    for user in users:
        ...
        # Store trajectory for charting
        user_trajectories[user["name"]] = history
...
if __name__ == "__main__":
    report, user_trajectories = summarize_metrics()
    output_dir = Path("experiments")
    output_path = output_dir / "evaluation_report.md"

    # Generate charts
    chart_files = generate_learning_charts(user_trajectories, output_dir)
    ...
    if chart_files:
        ...
        for chart_file in chart_files:
            user_name = chart_file.replace("learning_trajectory_", "").replace(".png", "").title()
            chart_section.append(f"### {user_name}")
            chart_section.append(f"![{user_name} Learning Trajectory]({chart_file})")
